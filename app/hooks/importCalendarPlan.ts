  ...
  reader.onload = (e) => {
    ...
    rows.forEach((row) => {
      ...
      if (value.startsWith('Регистрационный номер:')) {
        meta.reg_number = value.replace('Регистрационный номер:', '').trim();
      }
      if (value.startsWith('Дата начала обучения:')) {
        meta.start_date = value.replace('Дата начала обучения:', '').trim();
      }
      if (value.startsWith('Дата окончания обучения:')) {
        meta.end_date = value.replace('Дата окончания обучения:', '').trim();
      }
    });
    
    const headerIndex = rows.findIndex((r) => r[0] === 'Курс');
    if (headerIndex === -1) {
